...
class SolViajeActBuilder(BaseNodeBuilder):
    
    def get_node_type(self) -> str:
        return NodeType.SOL_VIAJE_ACT.value
    
    def get_ids_by_parent(self, parent_id, parent_type: str) -> List:
        print(f"[DEBUG BUILDER SV-ACT] parent_id={parent_id}, parent_type='{parent_type}'")
        ...
        if parent_type == NodeType.ACTIVIDAD.value:
            ...
            print(f"[DEBUG BUILDER SV-ACT] Encontradas: {len(ids)}, IDs: {ids}")
            return ids
        return []
    ...
    def get_parent_id(self, child_id, parent_type: str) -> Optional[int]:
        try:
            SolicitudViaje = apps.get_model('spme_monitoreo', 'SolicitudViaje')
            obj = SolicitudViaje.objects.only('actividad_id').get(id=child_id)
            if parent_type == NodeType.ACTIVIDAD.value:
    ...
    def build(self, node_id, nivel=0, es_nodo_objetivo=False, depth_remaining=None, build_context=None) -> TreeNode:
        print(f"[DEBUG BUILDER SV-ACT] Construyendo ID={node_id}")
        try:
            SolicitudViaje = apps.get_model('spme_monitoreo', 'SolicitudViaje')
            ...
            print(f"[DEBUG BUILDER SV-ACT] Estado: {estado_consolidado}")
            return self._create_node(NodeType.SOL_VIAJE_ACT.value, obj.id, nivel, datos, es_nodo_objetivo, build_context=build_context)
        except Exception as e:
            print(f"[DEBUG BUILDER SV-ACT] Error: {e}")
