...
class SolPagoDirectoSubBuilder(BaseNodeBuilder):
    
    def get_node_type(self) -> str:
        return NodeType.SOL_PAGO_DIRECTO_SUB.value
    
    def get_ids_by_parent(self, parent_id, parent_type: str) -> List:
        print(f"[DEBUG BUILDER SPD-SUB] parent_id={parent_id}, parent_type='{parent_type}'")
        ...
        if parent_type == NodeType.TAREA.value:
            ...
            print(f"[DEBUG BUILDER SPD-SUB] Encontradas: {len(ids)}, IDs: {ids}")
            return ids
        return []
    ...
    def get_parent_id(self, child_id, parent_type: str) -> Optional[int]:
        try:
            SolicitudPagoDirecto = apps.get_model('spme_monitoreo', 'SolicitudPagoDirecto')
            obj = SolicitudPagoDirecto.objects.only('tarea_id').get(id=child_id)
            if parent_type == NodeType.TAREA.value:
                return obj.tarea_id
        except Exception:
            pass
    ...
    def build(self, node_id, nivel=0, es_nodo_objetivo=False, depth_remaining=None, build_context=None) -> TreeNode:
        print(f"[DEBUG BUILDER SPD-SUB] Construyendo ID={node_id}")
        try:
            SolicitudPagoDirecto = apps.get_model('spme_monitoreo', 'SolicitudPagoDirecto')
            ...
            print(f"[DEBUG BUILDER SPD-SUB] Estado: {estado_consolidado}")
            return self._create_node(NodeType.SOL_PAGO_DIRECTO_SUB.value, obj.id, nivel, datos, es_nodo_objetivo, build_context=build_context)
        except Exception as e:
            print(f"[DEBUG BUILDER SPD-SUB] Error: {e}")
