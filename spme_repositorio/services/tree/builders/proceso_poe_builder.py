...
class ProcesoPOEBuilder(BaseNodeBuilder):
    
    def get_node_type(self) -> str:
        return NodeType.PROCESO_POE.value
    
    def get_ids_by_parent(self, parent_id, parent_type: str) -> List:
        print(f"[DEBUG BUILDER PROCPOE] parent_id={parent_id}, parent_type='{parent_type}'")
        if parent_type == NodeType.PRODUCTO_OE.value:
            ids = list(Proceso.objects.filter(
                producto_oe_id=parent_id
            ).values_list('id', flat=True))
            print(f"[DEBUG BUILDER PROCPOE] Encontrados: {len(ids)}, IDs: {ids}")
            return ids
        return []
    
    def get_parent_id(self, child_id, parent_type: str) -> Optional[int]:
        try:
            obj = Proceso.objects.only('producto_oe_id').get(id=child_id)
            if parent_type == NodeType.PRODUCTO_OE.value:
                return obj.producto_oe_id
