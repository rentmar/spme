# services/rendicion_cuentas_tarea_pei_pdf.py
...
class RendicionCuentasTareaPeiPDFGenerator(BasePDFGenerator):
    """
    Generador específico para Rendición de Cuentas de Tarea PEI
    Para rendiciones que tienen tarea asociada (obj.tarea existe)
    """
    
    def __init__(self):
        super().__init__()
        self.template_name = 'spme_impresiones/rendicion_cuentas_tarea_pei_template.html'
    
    def prepare_context(self, obj):
        """
        Prepara el contexto específico para RendicionCuentasActPei con tarea
        """
        ...
        if not obj.tarea:
            raise ValueError("Este generador es solo para rendiciones con tarea")
        ...
        context = {
            ...
            'numero_formulario': obj.numeroFormulario or f"RC-TAREA-{obj.id:04d}",
            ...
            'codigo_tarea': tarea.codigo if tarea else "N/A",
            ...
            # Datos de la actividad/rendición
            'fecha_actividad': obj.fechaActividad.strftime('%d/%m/%Y') if obj.fechaActividad else "No especificada",
            'descripcion_actividad': obj.descripcionActividad or (tarea.descripcion if tarea else "No especificada"),
    ...
    def generate_filename(self, obj):
        ...
        numero = obj.numeroFormulario or f"RC-TAREA-{obj.id:04d}"
        numero_limpio = "".join(c for c in numero if c.isalnum() or c in ['-', '_'])
        return f"Rendicion_Cuentas_Tarea_PEI_{numero_limpio}.pdf"