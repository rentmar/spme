# services/solicitud_pago_directo_act_pei_pdf.py
from .pdf_base import BasePDFGenerator
from spme_monitoreo.models import SolicitudPagoDirectoActPei
import json

class SolicitudPagoDirectoActPeiPDFGenerator(BasePDFGenerator):
    """
    Generador específico para Solicitud de Pago Directo de Actividad PEI
    """
    
    def __init__(self):
        super().__init__()
        self.template_name = 'spme_impresiones/solicitud_pago_directo_act_pei_template.html'
    
    def prepare_context(self, obj):
        """
        Prepara el contexto específico para SolicitudPagoDirectoActPei
        """
        ...
        context = {
            ...
            'numero_formulario': obj.numeroFormulario or f"SPD-ACT-{obj.id:04d}",
    ...
    def generate_filename(self, obj):
        ...
        numero = obj.numeroFormulario or f"SPD-ACT-{obj.id:04d}"
        numero_limpio = "".join(c for c in numero if c.isalnum() or c in ['-', '_'])
        return f"Solicitud_Pago_Directo_Actividad_PEI_{numero_limpio}.pdf"