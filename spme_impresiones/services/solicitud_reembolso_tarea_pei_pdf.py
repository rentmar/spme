# services/solicitud_reembolso_tarea_pei_pdf.py
...
class SolicitudReembolsoTareaPeiPDFGenerator(BasePDFGenerator):
    """
    Generador específico para Solicitud de Reembolso de Tarea PEI
    Para solicitudes que tienen tarea asociada (obj.tarea existe)
    """
    
    def __init__(self):
        super().__init__()
        self.template_name = 'spme_impresiones/solicitud_reembolso_tarea_pei_template.html'
    
    def prepare_context(self, obj):
        """
        Prepara el contexto específico para SolicitudReembolsoActPei con tarea
        """
        ...
        context = {
            ...
            'numero_formulario': obj.numeroFormulario or f"SR-TAREA-{obj.id:04d}",
            ...
            # Datos de la solicitud (desde la tarea/actividad)
            'fecha_ejecucion': obj.fechaRealizacionActividad.strftime('%d/%m/%Y') if obj.fechaRealizacionActividad else "No especificada",
            'descripcion_actividad': obj.descripcion_actividad or (tarea.descripcion if tarea else "No especificada"),
            ...
            # Presupuesto de actividad (contexto)
            ...
            'subtipo_documento': 'SUBACTIVIDAD PEI',
    ...
    def generate_filename(self, obj):
        ...
        numero = obj.numeroFormulario or f"SR-TAREA-{obj.id:04d}"
        numero_limpio = "".join(c for c in numero if c.isalnum() or c in ['-', '_'])
        return f"Solicitud_Reembolso_Tarea_PEI_{numero_limpio}.pdf"