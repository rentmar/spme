# serializers/rendicion_cuentas_usuario_serializers.py
# spme/spme_validaciones/serializers/rendicion_cuentas_usuario_serializers.py
...
class SolicitudOrigenSerializer(serializers.Serializer):
    """Serializer para la solicitud origen de la rendición"""
...
class RendicionCuentasUsuarioSerializer(serializers.Serializer):
    # Identificación
    id = serializers.IntegerField()
    numeroForm = serializers.CharField(allow_null=True, allow_blank=True)
    ...
    cpteDiario = serializers.CharField(allow_blank=True)
    fechaDesembolso = serializers.DateField(allow_null=True)
    montoAsignado = serializers.FloatField()
    montoDescargado = serializers.FloatField()
    saldo = serializers.FloatField()
    fechaActividad = serializers.DateField(allow_null=True)
    fechaRendicion = serializers.DateField(allow_null=True)
    descripcionActividad = serializers.CharField(allow_blank=True)
    lugarActividad = serializers.CharField(allow_blank=True)
    lugarRendicion = serializers.CharField(allow_blank=True)
    
    # Solicitud origen
    solicitudOrigen = SolicitudOrigenSerializer(allow_null=True)
    
    # Actividad y Tarea
    actividad = ActividadSerializer(allow_null=True)
    tarea = TareaSerializer(allow_null=True)
    
    # Datos generales (compatibilidad)
...
class RendicionCuentasUsuarioResponseSerializer(serializers.Serializer):
    tipo = serializers.CharField(allow_blank=True)
    total = serializers.IntegerField()
    solicitudes = RendicionCuentasUsuarioSerializer(many=True)
