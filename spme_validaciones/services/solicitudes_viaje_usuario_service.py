# spme/spme_validaciones/services/solicitudes_viaje_usuario_service.py
from typing import Dict, List, Any, Optional
from datetime import datetime
import logging

from ..repositories.solicitudes_viaje_usuario_repository import (
    SolicitudesViajeUsuarioRepository
)

logger = logging.getLogger(__name__)


class SolicitudesViajeUsuarioService:
    """
    Servicio de lógica de negocio especializado en Solicitudes de Viaje.
    """
    ...
    def __init__(self, repository: Optional[SolicitudesViajeUsuarioRepository] = None):
        self.repository = repository or SolicitudesViajeUsuarioRepository()
        ...
        try:
            ...
            for solicitud in solicitudes:
                try:
                    ...
                    logger.error(
                        f"Error procesando solicitud viaje {solicitud.id}: {str(e)}",
                        exc_info=True
                    )
                    continue
            ...
            logger.error(
                f"Error obteniendo solicitudes viaje para usuario {usuario_id}: {str(e)}",
                exc_info=True
            )
            raise
    ...
    def _construir_registro_solicitud(
        ...
        """Construye el registro unificado para una solicitud de viaje."""
        return {
            # Identificación
            'id': solicitud.id,
            ...
            # Datos específicos de Solicitud de Viaje
            'evento': solicitud.evento or '',
            'lugarEvento': solicitud.lugarEvento or '',
            'fechaEvento': solicitud.fechaEvento,
            'organizador': solicitud.organizador or '',
            'institucionesParticipantes': solicitud.institucionesParticipantes or '',
            'justificacionAsistencia': solicitud.justificacionAsistencia or '',
            'quienCubreGastos': solicitud.quienCubreGastos or '',
            'fondosUnitas': solicitud.fondosUnitas or '',
            'tareasPrevias': solicitud.tareasPrevias or '',
    ...
    def obtener_validaciones_como_validador(self, usuario_id: int) -> List[Dict]:
        """
        Obtiene TODAS las validaciones de viajes donde el usuario es validador.
        ...
        try:
            ...
            logger.error(f"Error validaciones viajes usuario {usuario_id}: {str(e)}", exc_info=True)
            return []