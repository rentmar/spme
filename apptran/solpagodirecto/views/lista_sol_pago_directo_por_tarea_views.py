...
from ..serializer.lista_sol_pago_directo_por_tarea_serializer import SolicitudPagoDirectoSerializer
from ..services.estado_solicitud_service import EstadoSolicitudService


class SolicitudesPagoDirectoPorActividadYTareaView(APIView):
    """Endpoint para obtener solicitudes de pago directo por actividad y tarea."""
    ...
    def get(self, request, id_actividad, id_tarea):
        ...
        queryset = SolicitudPagoDirecto.objects.filter(
            actividad_id=id_actividad,
            tarea_id=id_tarea
        ).select_related(
            'usuario', 'formaPago', 'actividad', 'tarea'
        ).prefetch_related(
            Prefetch('validaciones', queryset=ValidacionSolicitudPagoDirecto.objects.select_related('usuarioValidador'))
        )
        ...
        base_stats = SolicitudPagoDirecto.objects.filter(actividad_id=id_actividad, tarea_id=id_tarea)
        ...
        serializer = SolicitudPagoDirectoSerializer(queryset, many=True)
