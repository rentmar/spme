...
from ..serializer.lista_sol_fondos_por_tarea_serializer import SolicitudFondosSerializer
from ..services.estado_solicitud_service import EstadoSolicitudService


class SolicitudesPorActividadYTareaView(APIView):
    """Endpoint para obtener solicitudes por actividad y tarea."""
    ...
    def get(self, request, id_actividad, id_tarea):
        ...
        queryset = SolicitudFondos.objects.filter(
            ...
            Prefetch('validaciones', queryset=ValidacionSolicitudFondos.objects.select_related('usuarioValidador'))
        )
        ...
        base_stats = SolicitudFondos.objects.filter(actividad_id=id_actividad, tarea_id=id_tarea)
        ...
        serializer = SolicitudFondosSerializer(queryset, many=True)
