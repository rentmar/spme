# spme/spme_tree_reporter/services/renderers/indicador_roe.py
"""
Renderer para el nodo 'indicadorroe'.
Indicador de Resultado del Objetivo Específico.
"""

from .base import BaseRenderer


class IndicadorROERenderer(BaseRenderer):
    """
    Renderer del nodo Indicador de Resultado OE.
    
    Misma estructura que los otros indicadores pero en nivel 4.
    La bitácora se integrará cuando tengamos el BitacoraRepository.
    """
    
    tipo_nodo = 'indicadorroe'
    nivel_heading = 4
    requiere_salto_pagina = False
    renderizar_hijos = False
    
    def get_titulo_seccion(self, nodo: dict) -> str:
        codigo = nodo.get('datos', {}).get('codigo', 'IND-ROE')
    ...
    def render(self, nodo: dict, context) -> None:
        ...
        # entradas = bitacora_repo.obtener_bitacora('indicadorroe', nodo['id'])
