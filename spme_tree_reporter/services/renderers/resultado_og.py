"""
Renderer para el nodo 'resultadoog'.
Genera: descripción, supuestos/riesgos, actividades vinculadas.
"""

from .base import BaseRenderer


class ResultadoOGRenderer(BaseRenderer):
    """
    Renderer del nodo Resultado del Objetivo General.
    
    Contenido que genera:
    - Encabezado con código del resultado
    - Descripción destacada
    - Tabla de Supuestos y Riesgos
    - Lista de actividades vinculadas (referencia)
    
    Sus hijos (indicadores ROG, procesos ROG) los renderiza el walker.
    """
    
    tipo_nodo = 'resultadoog'
    nivel_heading = 2
    requiere_salto_pagina = False
    renderizar_hijos = True
    
    def get_titulo_seccion(self, nodo: dict) -> str:
        codigo = nodo.get('datos', {}).get('codigo', 'ROG')
    ...
    def _render_supuestos_riesgos(self, datos: dict, context) -> None:
        """Tabla con supuestos y riesgos del resultado."""
    ...
    def _render_actividades(self, nodo: dict, context) -> None:
        """Lista de actividades vinculadas directamente al resultado."""
