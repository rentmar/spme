"""
Renderer para el nodo 'objetivoespecificoog'.
Genera: descripción, supuestos/riesgos y actividades vinculadas.
"""

from .base import BaseRenderer


class ObjetivoEspecificoOGRenderer(BaseRenderer):
    """
    Renderer del nodo Objetivo Específico.
    
    Contenido que genera:
    - Encabezado con código del OE
    - Descripción destacada
    - Tabla de Supuestos y Riesgos
    - Lista de actividades vinculadas
    
    Sus hijos (indicadores, resultados, productos, procesos) los renderiza el walker.
    """
    
    tipo_nodo = 'objetivoespecificoog'
    nivel_heading = 2
    requiere_salto_pagina = False
    renderizar_hijos = True
    
    def get_titulo_seccion(self, nodo: dict) -> str:
        codigo = nodo.get('datos', {}).get('codigo', 'OE')
        return f'OBJETIVO ESPECÍFICO ({codigo})'
    ...
    def _render_actividades(self, nodo: dict, context) -> None:
        """Lista de actividades vinculadas al objetivo específico."""
